...
const INVITATION_SERVICE_TS = join(__dirname, '..', '..', 'org', 'invitation-service.ts');
const AUDIT_SERVICE_TS = join(__dirname, '..', '..', 'audit', 'admin-audit-service.ts');
const MIG_041 = join(__dirname, '..', '..', '..', 'migrations', '041_invitation_owner_swap.sql');
...
describe('WYREAI-118 + 119 admin create-org + owner-swap discipline (source-grep regression guards)', () => {
  ...
  describe('Migration 041 schema invariants', () => {
    it('adds invite_type column with default member_join', () => {
      expect(migSrc).toMatch(/ADD COLUMN IF NOT EXISTS invite_type[\s\S]*?DEFAULT 'member_join'/);
    });

    it('adds swap_from_user_id with FK to users + ON DELETE SET NULL', () => {
      expect(migSrc).toMatch(/ADD COLUMN IF NOT EXISTS swap_from_user_id[\s\S]*?REFERENCES users\(id\)[\s\S]*?ON DELETE SET NULL/);
    });

    it('enforces invite_type CHECK constraint (allowlist)', () => {
      expect(migSrc).toMatch(/CHECK[\s\S]*?invite_type IN \(\s*'member_join',\s*'owner_swap_to_invited'\s*\)/);
    });

    it('enforces swap-consistency CHECK: owner_swap_to_invited iff swap_from_user_id IS NOT NULL', () => {
      expect(migSrc).toMatch(/invite_type = 'owner_swap_to_invited'\s+AND swap_from_user_id IS NOT NULL/);
      expect(migSrc).toMatch(/invite_type <> 'owner_swap_to_invited'\s+AND swap_from_user_id IS NULL/);
    });
  });
  ...
  describe('POST /admin/orgs route discipline', () => {
    it('route registered with requireAdminMutation gate', () => {
      ...
      expect(adminSrc).toMatch(/app\.post<[\s\S]*?>\(\s*'\/admin\/orgs'\s*,[\s\S]*?if\s*\(\s*!requireAdminMutation\(/);
    });
    ...
    it('admin sub from request.auth0User used as stub-owner (NOT invited email)', () => {
      ...
      expect(adminSrc).toMatch(/orgService\.createOrg\(\s*\n?\s*name,\s*\n?\s*adminSub,/);
    });
  ...
  describe('InvitationService owner-swap branch discipline', () => {
    ...
    it('createInvitation throws on owner_swap_to_invited without swapFromUserId (paired-canary)', () => {
      expect(invSrc).toMatch(/inviteType === 'owner_swap_to_invited' && !swapFromUserId/);
      expect(invSrc).toMatch(/throw new Error\(/);
    });

    it('createInvitation INSERT carries invite_type + swap_from_user_id', () => {
      expect(invSrc).toMatch(/INSERT INTO org_invitations[\s\S]*?invite_type,\s*swap_from_user_id/);
    });

    it('acceptInvitation branches on invitation.inviteType', () => {
      expect(invSrc).toMatch(/if\s*\(\s*invitation\.inviteType === 'owner_swap_to_invited'\s*\)/);
    });
    ...
    it('owner-swap branch uses NARROWED DELETE (filter on swap_from_user_id), NOT blanket-DELETE', () => {
      ...
      expect(invSrc).toMatch(/DELETE FROM org_members[\s\S]*?WHERE org_id = \$\{invitation\.orgId\}[\s\S]*?AND role = 'owner'[\s\S]*?AND user_id = \$\{fromUserId\}[\s\S]*?AND user_id != \$\{userId\}/);
    });
    ...
    it('owner-swap branch does NOT include the customer-only structural assertion', () => {
      ...
      const swapBranchStart = invSrc.indexOf("if (invitation.inviteType === 'owner_swap_to_invited')");
      ...
      expect(swapBranch).not.toMatch(/orgRow\[0\] && orgRow\[0\]\.type !== 'customer'/);
    ...
    it('toInvitation maps invite_type + swap_from_user_id to the OrgInvitation type', () => {
      expect(invSrc).toMatch(/inviteType:\s*row\.invite_type as InvitationType/);
