...
describe('monthlyTotalCents + composedBillLine', () => {
  // decision-of-record §5: 5 humans, 2 agents → $700/mo.
  it('reconciles base + perSeat × billableSeats', () => {
    const sb = mockSeatBilling(5, 2);
    expect(monthlyTotalCents(sb)).toBe(70_000);
    expect(composedBillLine(sb)).toBe('$600 base + 5 seats × $20 = $700/mo');
  });
  it('agent-heavy org — billed agents add to the line (5h/4a → $740)', () => {
    expect(composedBillLine(mockSeatBilling(5, 4)))
      .toBe('$600 base + 7 seats × $20 = $740/mo');
  });
  it('smallest paid org — 1 human, 0 agents → $620', () => {
    expect(composedBillLine(mockSeatBilling(1, 0)))
      .toBe('$600 base + 1 seat × $20 = $620/mo');
  });
});

describe('seatBreakdownLine — the inclusion-explicit seat line', () => {
  it('all agents within the inclusion — "N of 2 included"', () => {
    expect(seatBreakdownLine(mockSeatBilling(5, 2)))
      .toBe('7 seats — 5 members + 2 agents (2 of 2 agent seats included)');
  });
  it('over the inclusion — "2 included, M billed"', () => {
    expect(seatBreakdownLine(mockSeatBilling(5, 4)))
      .toBe('9 seats — 5 members + 4 agents (2 included, 2 billed)');
  });
  it('one agent — singular agent-seat copy', () => {
    expect(seatBreakdownLine(mockSeatBilling(5, 1)))
      .toBe('6 seats — 5 members + 1 agent (1 of 1 agent seat included)');
  });
  it('zero agents — no agent clause', () => {
    expect(seatBreakdownLine(mockSeatBilling(5, 0))).toBe('5 seats — 5 members');
  });
  it('singular member', () => {
    expect(seatBreakdownLine(mockSeatBilling(1, 0))).toBe('1 seat — 1 member');
  });
});

describe('agentSeatConsentCopy — truthful per the inclusion', () => {
  it('adding agent #1 (0 agents now) — included, $0', () => {
    expect(agentSeatConsentCopy(mockSeatBilling(5, 0), { trialing: false }))
      .toBe('Adds 1 agent seat — included in your plan, $0.');
  });
  it('adding agent #2 (1 agent now) — still included', () => {
    expect(agentSeatConsentCopy(mockSeatBilling(5, 1), { trialing: false }))
      .toContain('included in your plan, $0');
  });
  it('adding agent #3 (2 agents now) — first billed agent, plain proration', () => {
    expect(agentSeatConsentCopy(mockSeatBilling(5, 2), { trialing: false }))
      .toBe('Adds 1 agent seat — $20/mo, prorated for the remainder of this cycle.');
  });
  it('adding a billed agent during a trial — charge framed at trial end', () => {
    expect(agentSeatConsentCopy(mockSeatBilling(5, 2), { trialing: true }))
      .toBe('Adds 1 agent seat — $20/mo, applied when your trial ends.');
  });
  it('never shows a computed dollar proration figure', () => {
    const copy = agentSeatConsentCopy(mockSeatBilling(5, 5), { trialing: false });
    expect(copy).not.toMatch(/\$\d+\.\d/); // no "$X.XX this cycle"
  });
});

describe('memberSeatConsentCopy — a human seat is always $20', () => {
  it('plain proration off-trial', () => {
    expect(memberSeatConsentCopy(mockSeatBilling(5, 0), { trialing: false }))
      .toBe('Adds 1 member seat — $20/mo, prorated for the remainder of this cycle.');
  });
  it('trial framing on-trial', () => {
    expect(memberSeatConsentCopy(mockSeatBilling(5, 0), { trialing: true }))
      .toBe('Adds 1 member seat — $20/mo, applied when your trial ends.');
  });
});
