...
describe("renderResellerCustomers", () => {
  it("renders the header with org name and active count", () => {
    const html = renderResellerCustomers(
      data([customer({}), customer({ id: "c2" })]),
    );
  ...
  it("opens the customer via a live link; legacy impersonate+more actions removed", () => {
    ...
    // Legacy disabled-with-dev-language Impersonate/More actions are gone (RC5 sweep).
    const disabled = html.match(/rc-action[^>]*disabled/g) ?? [];
    expect(disabled.length).toBe(0);
    expect(html).not.toContain("lands in a follow-up");
  });
  ...
  it("escapes customer name + subdomain (no HTML injection)", () => {
    const html = renderResellerCustomers(
      data([customer({ name: "<script>x</script>", subdomain: 'a"b' })]),
    );
  ...
  it("exposes data-name and data-plan on rows for client-side filtering", () => {
    const html = renderResellerCustomers(
      data([customer({ name: "Acme MSP", plan: "pro" })]),
    );
